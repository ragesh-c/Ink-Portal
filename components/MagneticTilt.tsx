...
// Detect touch-primary devices once at module load — avoids per-render checks.
// matchMedia('(hover: none)') is true on phones/tablets that lack a true pointer.
...
const MagneticTilt: React.FC<MagneticTiltProps> = ({
  ...
  // Skip all tilt logic on touch devices — mousemove never fires there anyway,
  // but this also prevents `will-change-transform` from promoting layers needlessly.
  const tiltDisabled = disabled || isTouchDevice;

  const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    if (tiltDisabled || !ref.current) return;
  ...
  return (
    <div
      ref={ref}
      className={`transition-transform duration-200 ease-out ${tiltDisabled ? '' : 'will-change-transform'} ${className}`}
